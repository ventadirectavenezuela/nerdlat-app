...
const LayoutHeader = ({ isAdmin, currentUser, onLoginClick, onRegisterClick, onLogout }) => {
  return (
    <header className="bg-[#2C3E50] text-white p-0 shadow-md sticky top-0 z-40 h-40 flex items-center justify-center overflow-hidden">
      <div className="container mx-auto flex items-center justify-between h-full px-4 sm:px-6 lg:px-8 max-w-full md:max-w-5xl">
        ...
        {!isAdmin && (
          <div className="flex items-center space-x-2 sm:space-x-4">
            {!currentUser ? (
              <>
                <button 
                  onClick={onLoginClick} // Conectado al handler de click
                  className="bg-[#FF6F00] text-white px-3 py-2 rounded hover:bg-[#E76400] text-sm whitespace-nowrap"
                >
                  Iniciar Sesión
                </button>
                <button 
                  onClick={onRegisterClick} // Conectado al handler de click
                  className="bg-green-600 text-white px-3 py-2 rounded hover:bg-green-700 text-sm whitespace-nowrap"
                >
                  Registrarse
                </button>
              </>
